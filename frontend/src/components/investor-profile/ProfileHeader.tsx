  ...
  return (
    <div className="relative mb-8 font-['Sora',sans-serif]">
      ...
      <div className="h-64 md:h-80 w-full bg-[#1c2b1e] relative overflow-hidden rounded-b-[40px] shadow-2xl flex items-center justify-center">
        ...
        {profile?.coverUrl ? (
          <div className="absolute inset-0 z-10 transition-opacity duration-700" style={{ backgroundImage: `url(${profile.coverUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
        ) : (
        ...
        <button onClick={() => coverInputRef.current?.click()} className="absolute bottom-6 right-8 bg-white/10 backdrop-blur-md hover:bg-white/20 text-white border border-white/20 px-4 py-2 rounded-xl text-[12px] font-bold transition-all active:scale-95 flex items-center gap-2 z-20 cursor-pointer">
          <Camera size={14} strokeWidth={2.5} /> Actualizar Portada
        </button>
      </div>
        ...
        <div className="relative -mt-20 flex flex-col md:flex-row items-end gap-6 md:gap-8">
          <div className="relative group">
            <div className="w-40 h-40 rounded-[48px] border-[6px] border-[#f4f7f4] bg-white shadow-2xl overflow-hidden flex items-center justify-center relative z-10">
              {profile?.avatarUrl ? (
                <div className="w-full h-full transition-opacity duration-500" style={{ backgroundImage: `url(${profile.avatarUrl})`, backgroundSize: 'cover', backgroundPosition: 'center' }} />
              ) : (
                <div className="w-full h-full bg-gradient-to-tr from-[#1c2b1e] to-[#2e7d32] flex items-center justify-center text-4xl font-black text-white">{initials}</div>
              )}
            </div>
            <button onClick={() => avatarInputRef.current?.click()} className="absolute inset-0 z-20 bg-[#1c2b1e]/60 text-white opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 rounded-[48px] cursor-pointer border-none">
            ...
            {profile?.identityVerified && (
              <div className="absolute -right-2 -bottom-2 z-30 bg-white p-1.5 rounded-2xl shadow-lg">
                <div className="bg-[#2e7d32] text-white p-1 rounded-lg"><CheckCircle2 size={18} strokeWidth={3} /></div>
              </div>
            )}
          </div>
          ...
          <div className="flex-1 pb-2 text-center md:text-left">
            <div className="flex flex-col md:flex-row md:items-center gap-2 mb-3">
              ...
              {profile?.displayName && (
                <span className="text-[14px] font-bold text-[#2e7d32] bg-emerald-50 px-3 py-1 rounded-lg">@{profile.displayName}</span>
              )}
            </div>
