...
let EntrepreneurCampaignRepository = class EntrepreneurCampaignRepository extends database_1.BaseRepository {
    ...
    async getFinancialProgressShared(campaignId) {
       ...
       LIMIT 10`, [campaignId]);
        const dailyProgressRows = await this.queryMany(`WITH daily_investments AS (
        SELECT 
      ...
      ORDER BY date ASC`, [campaignId]);
        const breakdownRows = await this.queryMany(`SELECT 
    ...
    async findCampaignDocuments(campaignId) {
        return this.queryMany(`SELECT * FROM campaign_documents WHERE campaign_id = $1 ORDER BY created_at DESC`, [campaignId]);
    }
    async findCampaignDocumentById(campaignId, docId) {
        return this.queryOne(`SELECT id, file_url FROM campaign_documents WHERE id = $1 AND campaign_id = $2`, [docId, campaignId]);
    }
    async deleteCampaignDocument(docId) {
